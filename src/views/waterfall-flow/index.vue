<template>
  <div ref="containerRef" :class="$style['waterfall-flow']">
    <box v-for="good in goods" :key="good.uuid" :good="good" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    function PBL() {
      ...
      //	2.获得屏幕可显示的列数
      var boxW = (boxs[0] as HTMLElement).offsetWidth;
      ...
      for (var i = 0; i < boxs.length; i++) {
        if (i < colsNum) {
          everyH[i] = (boxs[i] as HTMLElement).offsetHeight;
        } else {
          var minH = Math.min.apply(null, everyH);//获得最小的列的高度
          var minIndex = getIndex(minH, everyH); //获得最小列的索引
          getStyle((boxs[i] as HTMLElement), minH, (boxs[minIndex] as HTMLElement).offsetLeft, i);
          everyH[minIndex] += (boxs[i] as HTMLElement).offsetHeight;//更新最小列的高度
        }
      }
    }
    /**
